...
static void Enable_Motor(RS05_HandleTypeDef *motor);
static void Disable_Motor(RS05_HandleTypeDef *motor);
static void Set_Homing(RS05_HandleTypeDef *motor);
static void Active_Feedback(RS05_HandleTypeDef *motor);
static void Select_MotorType(RS05_HandleTypeDef *motor, RS05_Type type);
static void Parameter_Writing(RS05_HandleTypeDef *motor, uint16_t index, uint8_t *data);
...
void RS05_MITMode(RS05_HandleTypeDef *motor, float target_pos, float target_speed, float Kp, float Kd, float torque)
{
    ...
    torque_data = (uint16_t)(((torque + 17.0f) / 34.0f) * 65535);
    pos_data = (uint16_t)(((target_pos + 12.57f) / 25.14f) * 65535);
    speed_data = (uint16_t)(((target_speed + 44.0f) / 88.0f) * 65535);
...
static void Active_Feedback(RS05_HandleTypeDef *motor)
{
    ...
    Can_Transmit(motor->hfdcan, id, tx_data, 7);
}
	...
	void RS05_Get_MotorData(RS05_HandleTypeDef *motor, FDCAN_RxHeaderTypeDef *p_msg, uint8_t *data_arry)
	{
		if (p_msg->IdType == FDCAN_EXTENDED_ID)
		{
			if (((p_msg->Identifier >> 8) & 0xFF) == motor->can_id)
			{
				...
				motor->speed = Uint_To_Float(motor->original_speed, -50.0f, 50.0f, 16);
				motor->torque = Uint_To_Float(motor->original_torque, -5.5f, 5.5f, 16);
